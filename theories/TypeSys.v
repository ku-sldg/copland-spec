...
| interp_asp_replace : forall p aid attrs args e' n nlt,
...
| interp_asp_wrap : forall p aid attrs args e' n nlt,
...
Proof.
  ...
  - ff.
    ...
    + rewrite <- Heqcall in *.
      invc H0.
      normer; eapply normalize_ev_done in Heqe; normer.
    + rewrite <- Heqcall in *.
      invc H0.
      normer; eapply normalize_ev_done in Heqe; normer.
...
| tc_appr_asp : forall p p' e aid args e' appr_id,
    canon_ev_rep G e = asp_evt p' (asp_paramsC aid args) e' ->
...
Ltac2 Notation "smper" := (ff (fun () => eauto with smp)).
...
Fixpoint context_supports_appr_type (G : GlobalContext) (t : EvidenceT) : Prop :=
  match t with
  | mt_evt => True
  | nonce_evt n => True
  | left_evt e' => context_supports_appr_type G e'
...
Fixpoint context_supports_appr (G : GlobalContext) p e t : Prop :=
  ...
  | asp NULL => True (* should not be used... maybe ever *)
