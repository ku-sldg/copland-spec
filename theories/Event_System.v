...
Fixpoint events_fix (G : GlobalContext) (p : Plc) (e : EvidenceT) (t : Term) (i : nat) 
  ...
  | att q t' => 
    ...
    e' <- eval G q e t' ;;
    ret ([req i p q e t'] ++ evs ++ [rpy (i + 1 + List.length evs) p q e'])
  | lseq t1 t2 => 
  ...
  | bseq s t1 t2 => 
    ...
    ret ([split i p] ++ evs1 ++ evs2 ++ [join (i + 1 + List.length evs1 + List.length evs2) p])
  | bpar s t1 t2 =>
    evs1 <- events_fix G p (splitEv_T_l s e) t1 (i + 2) ;;
    evs2 <- events_fix G p (splitEv_T_r s e) t2 (i + 2 + List.length evs1) ;;
    let loc := i + 1 in
    ret ([split i p] ++ [cvm_thread_start loc loc p (splitEv_T_r s e) t2] ++ evs1 ++ 
...
Proof.
  ...
  - generalizeEverythingElse t; induction t;
    simpl in *; intuition;
    ffa; econstructor; eauto.
Qed.
    
Lemma events_range: forall G t p e evs i,
  events G (cop_phrase p e t) i evs ->
  events_size G p e t = ret (length evs).
Proof.
